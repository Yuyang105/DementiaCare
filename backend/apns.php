...
if(isset($_POST['cancel_msg'])) {
  try
  {
    ...
    $sql = "SELECT Users.token FROM pair INNER JOIN Users ON pair.Caregiver = Users.Email WHERE pair.Patient = '$_POST[user]'";
    $result = mysql_query($sql, $con);
    $num = mysql_num_rows($result);

    while ($x = mysql_fetch_array($result)) {
      $apns->connect(); //连接
      $apns->addDT("$x[token]");  //加入deviceToken
      $apns->setText($_POST[cancel_msg]);  //发送内容
      $apns->setBadge(1);  //设置图标数
      $apns->setSound('keys.m4r');  //设置声音
  ...
  $sql = "SELECT Users.token, Users.Name FROM pair INNER JOIN Users
              ON pair.Caregiver = Users.Email WHERE pair.Patient = '$_POST[user]'";
  $result = mysql_query($sql, $con);
  $num = mysql_num_rows($result);
  ...
  while ($x = mysql_fetch_array($result)) {
    ...
    $apns->setText("Emergency Alert! Sent by your associated patient $x[Name].");  // Content
    $apns->setBadge(1);  // Set badge number
    $apns->setSound('iphone_alarm.caf');  // Set ringtone
    $apns->setExpiry(3600);  // Set expriy time
    $apns->setCP('custom operation',array('type' => '1','url' => 'http://www.google.com.hk'));  //自定义操作
    $apns->send();  // Send
    echo ' Emergency message sent ok';
  }
    }catch(Exception $e)
    {
      echo $e;
